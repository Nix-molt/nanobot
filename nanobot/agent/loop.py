    ...
    async def _process_message(self, msg: InboundMessage, session_key: str | None = None) -> OutboundMessage | None:
        ...
        if final_content is None:
            if iteration >= self.max_iterations:
                # Ask the model for a status update before giving up
                messages.append({"role": "user", "content": (
                    f"You've reached the {self.max_iterations}-iteration limit. "
                    "Do NOT use any tools. Reply with a brief summary: "
                    "what you accomplished, what's still left to do, and any blockers. "
